...
**Важно:** если ссылки в браузере ведут на `galiulin.github.io`
вместо локального сервера, передайте `--baseURL`:

```bash
hugo server --baseURL http://localhost:1313/
```

Это поведение Hugo по умолчанию: `baseURL` в `hugo.toml` указывает
на продакшен-домен, и Hugo при разработке рендерит абсолютные URL
по нему. С флагом `--baseURL` все внутренние ссылки будут указывать
на локальный сервер.
